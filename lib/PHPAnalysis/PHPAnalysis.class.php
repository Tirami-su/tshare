...
class PhpAnalysis
{
	 ...
	 * @param $source_words 词典库数组
	 * 注意, 需要PHP开放足够的内存才能完成操作
	 * @return void
	 */
	public function MakeDict($source_words, $target_file = '')
	{

		$target_file = ($target_file == '' ? $this->mainDicFile : $target_file);
		$allk        = array();

		foreach($source_words as $row) {
			$a = $row->getA(); $w = $row->getW(); $r = $row->getR();
