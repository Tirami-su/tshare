> 网站地址 <http://tshare.top>，目前仅向哈尔滨工业大学三个校区的师生开放注册
...
2. 把文件夹当作集合，搜索结果列表包含文件夹下的文件夹或文件
    ...
    缺点：文件夹下的所有内容都需要一个准确具体的命名，有些情况下会有无意义的命名，比如照片形式的资料，照片名称是时间
		

可分方案用户的搜索成本低，能迅速找到想要的资料，但不可分方案也有其应用场景，比如用户想要某门课程所有试卷，而这些试卷都包含在一个文件夹下，则可以一键下载而不用逐个下载

综上考虑，采用两种方案结合的方法
- 列表顶部提供一个开关，选择是否可分——用户依据自己的需求选择一种搜索方案
- 对于列表中的文件项，提供点击预览文件的功能
- 给予列表中的文件夹项（在不可分的情况下）相比于文件项更大的空间，显示目录接结构，点击进入详情页后，提供点击预览文件的功能——解决冗余和模糊问题
- 属于文件夹下的文件，其详情页提供其根目录链接——这样在可分情况下，也能下载整个文件夹
- 图片的上一级目录视为不可分文件夹——对常见的具有无意义名称的文件做针对性处理

在不可分情况下，搜索依据按权重降序包括
- 文件夹名称以及其包含的所有文件夹、文件的名称
- 文件夹描述（去系动词，介词，连词等无意义词）

在可分情况下，搜索依据按权重降序包括
- 文件路径（文件名称和所有上级目录的名称）
- 文件描述（只有单独上传的才可能有，如果属于文件夹下，则一定没有描述）
- 文件内容（需要事先提取）
...
#### 积分系统
积分相当于虚拟货币，是刺激用户活跃度的重要手段

积分的用处：主要是提高曝光率，影响力，比如提高寻物的排序，闲置交易排序，拼单的排序，表白墙的排序，另外还可以参与抽奖等
积分的获取方式：注册赠送，上传资料，回答问题且得到采纳，每月根据资料的下载情况、评分的积分奖励，以及向他人索要
#### 宣传和商业推广
美观不影响网站浏览体验的广告位
